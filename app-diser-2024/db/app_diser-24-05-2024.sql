...
-- Procedures
--
CREATE PROCEDURE `sp_v1_get_reporte_enc_01` (`codmod` VARCHAR(10))   BEGIN
SELECT *, i.id AS 'idInstrumento' FROM tbl_encuesta_01 i
INNER JOIN tbl_mse m on i.cod_mod = m.cod_mod
WHERE i.cod_mod = codmod;
END$$

CREATE PROCEDURE `sp_v1_get_reporte_enc_02` (`codmod` VARCHAR(10))   BEGIN
SELECT *, i.id AS 'idInstrumento' FROM tbl_encuesta_02 i
INNER JOIN tbl_mse m on i.cod_mod = m.cod_mod
WHERE i.cod_mod = codmod;
END$$

CREATE PROCEDURE `sp_v1_get_reporte_inst_01` (`cmod` CHAR(10))   BEGIN
...
CREATE PROCEDURE `sp_v1_set_login` (`codmod` VARCHAR(7))   BEGIN
